...
def get_from_llm(module_params):
    ...
    try:
        ...
        #推理模式点击
        tl_ele = tab('.^zero-button flex flex-y-center flex-x-center')
        if 'selected' in tl_ele.attr('class'):
            logging.error("当前为选中状态，无效点击")
        else:
            logging.error("当前没有选中，需要选中")
            tl_ele.click()

        #联网功能去掉
        tl_ele = tab('.^online-button flex flex-y-center flex-x-center')
        if 'selected' in tl_ele.attr('class'):
            logging.error("当前为选中状态，点击取消联网")
            tl_ele.click()
        else:
            logging.error("当前没有选中，无需取消联网")




        # 输入搜索框内容
        inputer = tab('.input-box-inner')
        inputer.click()
        inputer.input(prompt)

        # 点击生成按钮
        gent_btn_ele = tab('.^enter_icon')
        time.sleep(3)
        gent_btn_ele.click()
        tab.listen.start("https://chatglm.cn/chatglm/mainchat-api/conversation/messages?")
        ...
        while time.time()-start_time < 240:
            if tab('.followup-container'):
                logging.error("生成成功")
                content = tab.eles('.markdown-body md-body tl')[-1].text
                return content
            time.sleep(.5)
...
if __name__ == '__main__':
    task_params = {
        'platform': 'baidu',  # 所属平台 #本地部署写poxiao或不写
        'module': 'baidu_search',
