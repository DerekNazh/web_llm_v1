...
def get_from_llm(module_params):
    ...
    try:
        ...
        tab.get('https://www.tongyi.com/qianwen')
        ...
        #判断网页是否加载成功
        result = re_check(tab.states.ready_state == 'complete',timeout=30)
        if not result:
        ...
        # 查看cookies是否过期
        if tab('@@class^sc-jMsorb@@text()=登录'):
        ...
        # 输入搜索框内容
        inputer = tab('.:ant-input-outlined textarea')
        inputer.click()
        inputer.input(prompt)

        # 开启监听
        tab.listen.start("https://api.tongyi.com/dialog/conversation")
        # 点击生成按钮
        gent_btn_ele = tab('.:operateBtn')
        ...
        #判断是否有发送出用户提示词
        if tab(f'@@class^bubble@@text()={prompt}'):
            logging.error('判断是否有发送出用户提示词')
        else:
            logging.error('判断是否有发送出用户提示词失败，可能没有发出')
            raise  Exception('判断是否有发送出用户提示词失败，可能没有发出')





        packet = tab.listen.wait(timeout=180)
        data = packet.response.body
        data_list = list(filter(lambda item: not item == '', data.split('\n')))
        buffer = ''
        for line in data_list:
            try:
                if line.startswith('data:'):
                    ...
                    if line_json['contentType'] == 'text':
                        # 判断是否是文本信息json流[0]['content']
                        contents = line_json.get('contents', None)
                        if contents:
                            ...
                            logging.error(f"该数据流为头信息{line}")
                            continue
            except JSONDecodeError:
                if line == '[DONE]':
                    logging.error("获取流式输出完毕")
                    return buffer
