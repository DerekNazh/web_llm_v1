...
def get_from_llm(module_params):
    ...
    try:
        ...
        tab.get('https://yuanbao.tencent.com/chat')
        # 设置cookies
        ...
        if not result:
            ...
            raise PageLoadTimeoutError(f"{acc_platform}_{acc_code}_网页加载缓慢,15秒没有加载出来")
        ...
        while time.time()-start_time<10:
            if tab('微信扫码登录', timeout=0) or tab('@@class=nick-info-name@@text()=未登录', timeout=0):
            ...
            logging.error("cookies没有过期，可继续执行")

        #取消联网搜索
        online_ele = tab('.:cos-space-ml-xxs')
        if online_ele:
        ...
        # 点击生成按钮
        gent_btn_ele = tab('#yuanbao-send-btn')
        time.sleep(3)

        if 'send-btn--disabled' not in gent_btn_ele.attr('class'):
        ...
        while time.time()-start_time<30:
            user_send_ele = tab(f'@@class=hyc-content-text@text()={prompt}')
            if user_send_ele:
                logging.error("发送成功")
                break
            else:
            ...
            logging.error("没有发送出用户提示词信息")
            raise  Exception("没有发送出用户提示词信息")



        start_time = time.time()
        while time.time() - start_time < 300:
            if user_send_ele.after('.^yb-icon iconfont-yb icon-yb-ic_like'):
                logging.error('加载数据完毕')
                return tab.eles('.hyc-common-markdown hyc-common-markdown-style')[-1].text
            else:
                time.sleep(1)


        else:
            raise  Exception("超时3000s，获取文本失败")
        #定时捕获接口
        # time.sleep(60)
        # # 开启监听
        # tab.listen.start("https://chat.baidu.com/csaitab/history/list?req_type=5")
        # tab.refresh()
        #
        # packet = tab.listen.wait(timeout=20)
        # data = packet.response.body
        # # data.historyData[3].msgList[0].chatResponse[0].data.message.content.generator[1].data.value
        # value = \
        # data['data']["historyData"][-1]["msgList"][0]["chatResponse"][0]["data"]["message"]["content"]["generator"][1][
        #     "data"]["value"]
