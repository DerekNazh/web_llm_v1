...
def get_from_llm(module_params):
    ...
    try:
        ...
        time.sleep(3)
        gent_btn_ele = tab('.^send-button')
        if 'active' in gent_btn_ele.attr('class'):
            logging.error("点击按钮可以被点击")
            gent_btn_ele.click()
        else:
        ...
        while time.time()-start_time<240:
            ...
            message_wrapper = tab.eles('.message-wrapper')[-1]
            tail_msg_ele = message_wrapper.after('.tool-bar')
            if tail_msg_ele:
                logging.error("生成完毕")
                # 判断元素内文本几秒内没有更改
                conversation_ele = tab('.van-checkbox-group')
                tail_conver_ele = conversation_ele.children()[-1]
                return tail_conver_ele('.^answer-cont').eles('t:p')[-1].text
...
if __name__ == '__main__':
    task_params = {
        'import_path': f'xiaoyi/xiaoyi_dp',
        'prompt': '今天北京多少度'
    }
